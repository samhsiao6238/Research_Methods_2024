...
2. 依據數據格式，生成模擬數據。
    ...
    # 年齡區間
    age_ranges = [
        '21 ~ 30歲', '31 ~ 40歲', '41 ~ 50歲', '51 ~ 60歲'
    ]
    age = np.random.choice(age_ranges, size=400)

    # 是否使用 Instagram，'是'中簽率 70%，'否'中簽率有 30%
    used_instagram = np.random.choice(
        ['是', '否'], size=400, p=[0.7, 0.3]
    )

    # 是否知道Instagram有「隱藏此貼文的按讚數」
    know_hidden_likes = np.where(
        used_instagram == '是',
        np.random.choice(['是', '否'], size=400),
        '否'
    )
    ...
    ig_usage_years = np.where(
        used_instagram == '是',
        np.random.choice(usage_years, size=400),
        '不適用'
    )
    ...
    ig_login_frequency = np.where(
        used_instagram == '是',
        np.random.choice(login_frequency, size=400),
        '不適用'
    )

    # 使用裝置時間
    device_usage_time = [
        '30分鐘或更少', '30分鐘到1小時', '1小時到3小時', '3小時以上'
    ]
    ig_device_usage_time = np.where(
        used_instagram == '是',
        np.random.choice(device_usage_time, size=400),
        '不適用'
    )
    ...
    social_responses = np.random.choice(
        social_interactions, size=(400, 15)
    )
    ...
    hidden_likes_responses_open = np.random.choice(
        frequency_responses, size=(400, 18)
    )
