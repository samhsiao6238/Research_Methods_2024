...
1. 建立並進入專案資料夾，這裡配合後續教程中使用的路徑，將示範的資料夾建立在桌面。
...
2. 編輯兩個腳本，`ex01_觀察數據.ipynb` 與 `ex02_模擬數據.ipynb`，功能如名稱描述，代碼如下。

    ```bash
    touch ex01_觀察數據.ipynb ex02_模擬數據.ipynb
    ```
...
2. 腳本二：`ex02_模擬數據.ipynb`，依據數據格式，生成模擬數據。
    ...
    data.to_excel(xlsx_path3, index=False)
