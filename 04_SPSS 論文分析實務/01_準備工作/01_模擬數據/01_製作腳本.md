...
1. 建立並進入專案資料夾，這裡配合後續教程中使用的路徑，將示範的資料夾建立在桌面；資料夾若已經存在則直接進入下一步。

    ```bash
    mkdir ~/Desktop/00_論文實作
    ```
...
3. 編輯兩個腳本，`ex01_觀察數據.ipynb`、`ex02_模擬數據.ipynb` 與 `ex03_具有重複觀察值.ipynb`，功能如名稱描述，代碼如下；建立文件後啟動 VSCode。

    ```bash
    touch ex01_觀察數據.ipynb ex02_模擬數據.ipynb ex03_具有重複觀察值.ipynb && code .
    ```

<br>

3. 腳本 `ex01_觀察數據.ipynb` 是用以觀察原始資料的內容；`ex02_模擬數據.ipynb` 是主要的模擬數據腳本，運行後，會建立兩個數據文件；`ex03_具有重複觀察值.ipynb` 則是基於前一個腳本生成的文件添加重複觀察值，用以實作重複觀察值的篩選。
...
_編輯 `ex03_具有重複觀察值.ipynb`_

<br>

1. 添加以下代碼加入重複觀察樣本，或修改原本樣本具有部分的重複觀察值；以下共有三類的重複觀察值，其中第一類重複是複製並添加三個樣本來實現。
    ...
    # 檢查並讀取當前資料夾中的文件
    if os.path.exists('simulated_data_with_duplicates.sav'):
        data, meta = pyreadstat.read_sav('simulated_data_with_duplicates.sav')
    elif os.path.exists('simulated_data_with_duplicates.xlsx'):
        data = pd.read_excel('simulated_data_with_duplicates.xlsx')
    else:
        raise FileNotFoundError("無法找到 simulated_data_with_duplicates.sav 或 simulated_data_with_duplicates.xlsx 文件")

    '''第一類重複樣本'''
    # 複製前（2）個樣本的所有數據
    duplicate_1 = data.iloc[0:2].copy()
    # 將重複樣本添加到數據集中，也就是兩個重複觀察值
    data = pd.concat([data, duplicate_1], ignore_index=True)

    '''第二類重複樣本'''
    # 複製第（14~16）樣本的人口資訊與社交焦慮數據
    duplicate_2 = data.iloc[13:16, :26].copy()
    # 將第（14~16）樣本的數據覆蓋到第（4~6）樣本
    # 這樣第（4~6）樣本在這些欄位上的數據將與第（14~16）樣本完全一致
    data.iloc[3:6, :26] = duplicate_2.values

    '''第三類重複樣本'''
    # 第（1~7）欄是人口資訊，第（26~44）欄是顯示點讚相關列，第（45~62）欄是隱藏點讚相關數據
    # 複製第（24~26）樣本的人口資訊及顯示/隱藏點讚數據
    duplicate_3 = data.iloc[
        23:26,
        np.r_[0:7, 25:44, 44:62]
    ].copy()
    # 將第（24~26）樣本的人口資訊及顯示/隱藏點讚數據覆蓋到第（7~9）樣本
...
_使用 Python 腳本進行重複觀察值的清洗_

<br>

1. 以下僅僅是提供刪除重複觀察值的代碼，無需加入以上腳本。
