...
2. 依據數據格式，生成模擬數據。
    ...
    # 設定每個年資段的比例
    usage_years_probabilities = [0.1, 0.2, 0.3, 0.2, 0.2, 0]
    ...
    # 設定每個頻率的比例
    login_frequency_probabilities = [0.2, 0.3, 0.3, 0.1, 0.1]
    ig_login_frequency = np.where(
        used_instagram == '是',
        np.random.choice(login_frequency, size=400, p=login_frequency_probabilities),
        '不適用'
    )
