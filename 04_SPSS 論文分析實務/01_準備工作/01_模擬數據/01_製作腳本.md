...
3. 腳本 `ex01_觀察數據.ipynb` 是用以觀察原始資料的內容；`ex02_模擬數據.ipynb` 是主要的模擬數據腳本，運行後，會建立兩個數據文件 `_complete`；`ex03_具有重複觀察值.ipynb` 則是基於前一個腳本生成的文件添加重複觀察值，用以實作重複觀察值的篩選，完成後會再添加兩個文件 `_duplicates`。
...
1. 添加以下代碼加入重複觀察樣本，或修改原本樣本具有部分的重複觀察值；以下共有三類的重複觀察值，其中第一類重複是複製並添加三個樣本來實現。
    ...
    # 檢查並讀取當前資料夾中的文件
    if os.path.exists('simulated_data_complete.sav'):
        data, meta = pyreadstat.read_sav('simulated_data_complete.sav')
    elif os.path.exists('simulated_data_complete.xlsx'):
        data = pd.read_excel('simulated_data_complete.xlsx')
    else:
        raise FileNotFoundError("無法找到 simulated_data_complete.sav 或 simulated_data_complete.xlsx 文件")
